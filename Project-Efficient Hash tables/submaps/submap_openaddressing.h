#include "../hash_tables/hash_table_openaddressing.h"
#include <thread>
#include <queue>
#include <mutex>
...
// same procedure as followed in chaining.

class ht_submap_openaddr : public hash_table_openaddr{
private:
	int num_threads_supported;
	thread* worker_threads;
	vector<thread> executer_threads;
	hash_table_openaddr* sub_maps;
...
public:
	...
	ht_submap_openaddr(int init_length_){
		...
		sub_maps = new hash_table_openaddr[num_threads_supported];
	...
	void execute(string mode,string key,string value=""){
		...
		if(not running_workers[id]){
			worker_threads[id] = thread(&ht_submap_openaddr::handle_thread,this,id);
	...
	void utility(string mode,string key,string value =""){
		usleep(10);
		thread executer(&ht_submap_openaddr::execute,this,mode,key,value);
